...
const getTrailScore = (map: number[][], pos: Vector2, goalsReached = new Set<string>()): number => {
  const currentHeight = map[pos.y][pos.x];

  if (currentHeight === 9) {
    goalsReached.add(pos.toString());
  }

  getValidNeighbors(map, pos, currentHeight).forEach((neighbour) => {
    getTrailScore(map, neighbour, goalsReached);
  });

  return goalsReached.size;
};
